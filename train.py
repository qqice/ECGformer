...
x_train, y_train = readucr('./dataset/mitbih_train.csv')
...
# 保存完整模型（包含结构和权重），而不仅仅是权重
checkpoint_filepath = './ckpts/best_model.keras'
model_checkpoint_callback = keras.callbacks.ModelCheckpoint(
    filepath=checkpoint_filepath,
    save_weights_only=False,  # 改为False以保存完整模型
...
'''
# 添加 labels 参数来指定所有可能的类别
report = classification_report(y_test, y_pred, labels=[0, 1, 2, 3, 4], target_names=['N','S', 'V','F','Q'], zero_division=0)
print(report)
...
#plt.plot()
plt.savefig('./results/loss_plot.jpg', format='jpg')
